...
import { Ruler, User, Stethoscope, Apple } from "lucide-react";
...
export function ConsultationFields({
    editValues, setEditValues, extraData, setExtraData, imc, layout, clinicalVariables, patientId, isUploadingPhoto, setIsUploadingPhoto
    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            ...
            <div className="col-span-1 flex flex-col gap-6">
                <div className="bg-white/[0.03] p-6 rounded-[2rem] border border-white/5 shadow-inner focus-within:border-nutrition-500/50 transition-all group">
                    ...
                    <input
                        ...
                        className="w-full text-4xl font-tech font-black text-white focus:outline-none transition-colors bg-transparent border-none p-0 placeholder:text-white/5"
                    />
                </div>
            ...
            {/* Column 2: Perímetros */}
            <div className="col-span-1 bg-white/[0.03] p-6 sm:p-8 rounded-[2rem] border border-white/5">
                        ...
                        return (
                            <div key={item.variable_id || item.header} className="space-y-1">
                                ...
                                <input
                                    ...
                                    className="w-full bg-white/5 border border-white/5 rounded-xl px-3 py-2 text-sm font-tech font-black text-white focus:ring-1 focus:ring-blue-500/50 outline-none transition-all placeholder:text-white/5"
                                />
                            </div>
                        );
                    })}
                        ...
                        return (
                            <div key={item.variable_id || item.header} className="space-y-1">
                                ...
                                <input
                                    ...
                                    value={v ? (extraData[v.code] || '') : ''}
                                    onChange={e => v && updateExtraData(v.code, e.target.value)}
                                    className="w-full bg-white/5 border border-white/5 rounded-xl px-3 py-2 text-sm font-tech font-black text-white focus:ring-1 focus:ring-purple-500/50 outline-none transition-all placeholder:text-white/5"
                                />
                            </div>
                        );
                    })}
                ...
                {layout.filter(l => l.section === 'findings' || l.section === 'recommendations').map(item => {
                    ...
                    return (
                        <div key={item.variable_id || item.header} className="bg-white/[0.03] p-6 rounded-[2rem] border border-white/5 flex-1 flex flex-col focus-within:border-white/10 transition-all group">
                            ...
                            <textarea
                                placeholder={isFinding ? "Hallazgos..." : "Sugerencias..."}
                                value={v ? (extraData[v.code] || '') : ''}
                                onChange={e => v && updateExtraData(v.code, e.target.value)}
                                className="w-full flex-1 bg-white/5 border border-white/5 rounded-2xl p-4 text-sm text-slate-300 focus:ring-1 focus:ring-white/10 outline-none resize-none min-h-[100px] custom-scrollbar placeholder:text-white/5"
                            ></textarea>
                        </div>
                    );
                })}
