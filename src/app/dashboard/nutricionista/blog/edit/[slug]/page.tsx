...
export default function EditPostPage({ params }: { params: { slug: string } }) {
    ...
    useEffect(() => {
        const loadInitialData = async () => {
            ...
            // 1. Check custom posts in localStorage
            ...
            const article = custom || BLOG_POSTS.find(p => p.slug === params.slug);
            ...
            if (article) {
                ...
                if (article.content) {
                    setBlocks(article.content.map((b: any, i: number) => ({
                        id: i.toString(),
                        type: b.type,
                        text: b.text
                    })));
                } else {
                    setBlocks([{ id: "1", type: "p", text: article.excerpt || "" }]);
                }
            }
        };
    ...
    const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (file) {
            ...
            reader.onloadend = () => {
                ...
                toast({ title: "Imagen actualizada", description: "La imagen se ha adjuntado correctamente." });
            };
    ...
    const handleSave = (newStatus: "published" | "draft" = status) => {
        if (!title.trim()) {
        ...
        const updatedPost = {
            id: params.slug, // Keep id or slug reference
            title,
            slug: params.slug, // Keep original slug for simplicity in this demo
            category,
            author: profile?.full_name || "Nutricionista",
            authorId: profile?.id || "unknown",
            date: newStatus === "published"
        ...
        const stored = localStorage.getItem('nutrigo_custom_blog_posts');
        let customPosts = stored ? JSON.parse(stored) : [];

        // Update or Add
        const index = customPosts.findIndex((p: any) => p.slug === params.slug);
        if (index > -1) {
        ...
        localStorage.setItem('nutrigo_custom_blog_posts', JSON.stringify(customPosts));
        ...
        toast({
            title: newStatus === "published" ? "¡Publicación actualizada!" : "Borrador actualizado",
        ...
        <div className="max-w-4xl mx-auto space-y-8 pb-20">
            ...
            <div className="flex items-center justify-between sticky top-4 z-50 bg-white/80 backdrop-blur-xl p-4 rounded-[32px] border shadow-2xl shadow-slate-200/50">
                <div className="flex items-center gap-4">
                    ...
                    <span className="text-sm font-black text-slate-800 uppercase tracking-widest hidden md:block">Editar Artículo</span>
                </div>
                <div className="flex items-center gap-2">
                    ...
                    >
                        <Save className="h-4 w-4" /> Actualizar Borrador
                    </Button>
                    <Button
                    ...
                    <Card className="rounded-[40px]">
                        <CardContent className="p-8 space-y-6">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 pt-4 border-t border-slate-50">
                                ...
                                <div className="space-y-4">
                                    <div className="space-y-1.5">
                                        <label className="text-[10px] font-black uppercase text-slate-400 tracking-widest">Imagen de Portada</label>
